...
"""Benchmark ring-attention P2P transports and VisualGen collectives.
...
Usage — ring P2P micro-benchmark (all transports, recommended first):
    torchrun --nproc-per-node=4 bench_nccl_ub.py --tier micro --backend all

Usage — NIXL ring transport only:
    TRTLLM_RING_TRANSPORT=nixl torchrun --nproc-per-node=4 bench_nccl_ub.py \\
        --tier micro --backend nixl

Usage — full pipeline (requires weights):
    torchrun --nproc-per-node=4 bench_nccl_ub.py \\
        --tier pipeline \\
...
def _bench_ring_p2p_nccl(group: dist.ProcessGroup, world_size: int,
                          slot_bytes: int, warmup: int, iters: int) -> Dict:
...
def _bench_ring_p2p_nixl(group: dist.ProcessGroup, world_size: int,
                          slot_bytes: int, warmup: int, iters: int) -> Dict:
    ...
    try:
        ...
        from tensorrt_llm._torch.disaggregation.base.agent import (
            MemoryDescs, RegMemoryDescs, TransferRequest,
        )
    except ImportError:
    ...
    try:
        reg = RegMemoryDescs(type="VRAM", descs=[(kv_bufs.data_ptr(), kv_bufs.nbytes, device_id, "")])
        agent.register_memory(reg)
    except Exception as e:
        return {"error": f"register_memory failed: {e}"}

    # All-gather base pointers.
    info = torch.tensor([kv_bufs.data_ptr(), kv_bufs.nbytes, device_id], dtype=torch.int64, device=device)
        ...
        for transport in backends:
            ...
            if rank == 0:
                if "error" in stat:
                    ...
                    if transport != "nccl" and nccl_key in results and "error" not in results[nccl_key]:
                        ratio = results[nccl_key]["mean_ms"] / stat["mean_ms"]
                        speedup = f"  ({ratio:.2f}x vs NCCL)"
                    print(f"    {label:40s}  mean={stat['mean_ms']:.3f} ms  "
            ...
            if rank == 0:
                if "error" in stat:
                    ...
                    if label_prefix == "ub" and nccl_key in results:
                        ratio = results[nccl_key]["mean_ms"] / stat["mean_ms"]
                        speedup = f"  ({ratio:.2f}x vs NCCL)"
                    print(f"  {full_name:40s}  mean={stat['mean_ms']:.3f} ms  "
                          f"median={stat['median_ms']:.3f} ms{speedup}")
...
def _write_markdown_report(results: Dict, metadata: Dict, path: str) -> None:
    ...
    lines.append(f"**Warmup:** {metadata.get('warmup', '?')}  |  **Iters:** {metadata.get('iters', '?')}")
    lines.append("")
    ...
    lines.append("## Notes")
    lines.append("")
    lines.append("- NIXL uses GDR (GPUDirect RDMA) for cross-node IB transfers; "
    ...
    lines.append("- The NIXL path includes a `dist.barrier` per step for correctness; "
...
def main():
    ...
    parser.add_argument(
        "--backend", choices=["nccl", "ub", "nixl", "all"], default="all",
    ...
    parser.add_argument("--out", type=str, default="bench_ring_transport_results.json",
                        help="Output JSON path (rank-0 only)")
    parser.add_argument("--report", type=str, default="bench_ring_transport_report.md",
    ...
    if rank == 0:
        ...
        print(f"world_size={world_size}  TRTLLM_RING_TRANSPORT={ring_transport}  "
